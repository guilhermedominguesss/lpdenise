...
// Counter component provided by user
export const Component = () => {
  const [count, setCount] = useState(0);

  return (
    <div className={cn("flex flex-col items-center gap-4 p-4 rounded-lg bg-black text-white")}>
      <h1 className="text-2xl font-bold mb-2">Component Example</h1>
      <h2 className="text-xl font-semibold">{count}</h2>
      <div className="flex gap-2">
        <button onClick={() => setCount((prev) => prev - 1)} className="px-4 py-2 border border-white rounded">-</button>
        <button onClick={() => setCount((prev) => prev + 1)} className="px-4 py-2 border border-white rounded">+</button>
      </div>
    </div>
  );
...
// ParticleWaves component provided by user in demo.tsx
const ParticleWaves = () => {
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden">
      ...
      <div className="absolute top-2 right-2 bg-black/80 border border-gray-600 rounded-lg p-4 text-white text-xs w-52 z-10">
        ...
        <button 
          onClick={() => window.parent.postMessage({ type: "OPEN_EXTERNAL_URL", data: { url: "https://rollout.dev" } }, "*")}
